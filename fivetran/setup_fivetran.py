...
GROUP_ID   = "{{PLACEHOLDER_GROUP_ID}}"   # populated after Step 1 runs
...
group    = api_call("POST", "/groups", {"name": "Myntra CLV Analytics"})
...
api_call("POST", "/destinations", {
    "group_id":         group_id,
    "service":          "snowflake",
    "region":           "US", # Defaulting to US, adjust if Snowflake is in EU/APAC
    "time_zone_offset": "0",
    "config": {
...
def create_connector(config_file, group_id):
    ...
    # Fivetran API expects schema_prefix in config for some sources, or it uses destination schema
    # For azure_sql_db, we map destination schema in the payload config if needed, 
    # but usually it's handled by the destination object. 
    # However, to ensure schema mapping:
    config["schema_prefix"] = connector_data['destination']['schema']
...
if __name__ == "__main__":
    connectors = [
        "fivetran/myntra_erp_connector.yaml",
        "fivetran/myntra_crm_connector.yaml",
        "fivetran/myntra_marketing_connector.yaml"
    ]
